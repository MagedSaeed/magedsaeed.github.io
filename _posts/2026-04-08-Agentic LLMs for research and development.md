...
**Skills can ship code too, not just prose.** While writing this article I built [`pptx-from-file`](https://github.com/MagedSaeed/magedsaeed.github.io/tree/main/.claude/skills/pptx-from-file), a project skill that turns a source file (pdf, markdown, docx, ipynb) into a polished `.pptx`. Its `SKILL.md` encodes taste — read the content for design signals first, confirm an outline before building, keep one accent color, ask before fetching web images — and sits next to a `template.py` helper that provides seven layouts and per-paragraph animations. Invocation is as plain as *"make a pptx from `<path>`"*. The point: a skill can bundle a small design system and a Python module alongside its instructions, not only a style guide. For a concrete look at the output, here is a [sample generated presentation](https://docs.google.com/presentation/d/1Sj1cGYbOHxdisaVoukaRvmffEW23VwA_-RXYVJkypxc/edit?usp=sharing) generated from this article.
